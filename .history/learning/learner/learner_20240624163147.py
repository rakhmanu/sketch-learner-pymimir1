...
def is_unique_solution(sketch, sketches):
    # Define your uniqueness criteria here
    for existing_sketch in sketches:
        if existing_sketch.policy == sketch.policy:
            return False
    return True
...
def learn_sketch_for_problem_class(
        ...
        for instance_data in instance_datas:
            ...
            for state_idx in instance_data.state_space.get_states().keys():
                ...
                if len(valid_solutions) == 0:
                    ...
                    # Only add sketch if it's not a unique solution
                    if not is_unique_solution(sketch, sketches):
                        sketches.add(sketch)
                    for state_id in unsolvable_states_from_solution(valid_solutions[0]):
    ...
    with change_dir("output"):
        ...
        all_deadend_states_removed = all(sketch.verify_unsolvable_states_removed(instance_data, asp_factory) for sketch in sketches)
