...
def learn_sketch_for_problem_class(
    ...
    if encoding_type == EncodingType.EXPLICIT:
        ...
        for instance_data in instance_datas:
            ...
            preprocessing_timer.stop()
            asp_timer.resume()
            # Iterate over initial state IDs
            for state_id in instance_data.state_space.get_states().items():
                # Retrieve the state object corresponding to state_id
                instance_data = instance_data.state_space.get_states()[state_id]
