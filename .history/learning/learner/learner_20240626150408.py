...
def learn_sketch_for_problem_class(
    ...
    # Set Clingo random seed via options
    clingo_options = ["--rand-freq=1", f"--seed={random_seed}"]
    ...
    if encoding_type == EncodingType.EXPLICIT:
        ...
        for instance_data in instance_datas:
            ...
            for state_idx in instance_data.state_space.get_states().keys():
                ...
                asp_factory = ASPFactory(encoding_type, enable_goal_separating_features, max_num_rules, options = clingo_options)
                    ...
                    if len(valid_solutions) > 1:
                        ...
                        if sketch not in sketches_per_state[state_idx]:
                            ...
                            if state_idx not in sketch_count_per_state:
                                sketch_count_per_state[state_idx] = 0
    ...
    with change_dir("output"):
        ...
        total_sketches = sum(sketch_count_per_state.values())
        print(f"Total sketches across all states: {total_sketches}")
        for state_idx, count in sketch_count_per_state.items():
            print(f"State {state_idx}: {count} sketches")
        ...
        if not sketches_per_state:
            print("No sketches found in sketches_per_state.")
        # Ensure `sketches` is populated from `sketches_per_state`
        sketches = {sketch for sketches in sketches_per_state.values() for sketch in sketches}
        if not sketches:
            print("No sketches found to write to files.")
        for state_idx in instance_data.state_space.get_states().keys():
            if state_idx not in sketches_per_state:
