        ...
        for instance_data in instance_datas:
            ...
            for state_idx in instance_data.state_space.get_states().keys():
                ...
                for idx, symbols in enumerate(valid_solutions):
                    ...
                    sketches.add(sketch)
                     dlplan_policy = ExplicitDlplanPolicyFactory().make_dlplan_policy_from_answer_set(symbols, domain_data)
                    sketch_instance = Sketch(dlplan_policy, width)

                    # Check if sketch_instance is not None before calling verify_unsolvable_states_removed
                    if sketch_instance is not None and sketch_instance.verify_unsolvable_states_removed(instance_data, asp_factory):
                        print("Deadend states are removed")
                    else:
                        print("Failed to remove deadend states")

                    sketches.add(sketch_instance)  
