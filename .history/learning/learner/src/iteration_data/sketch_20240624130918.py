...
from learner import unsolvable_states_from_solution
...
class Sketch:
    ...
    def verify_unsolvable_states_from_solution():
        symbols = [
            clingo.Function("unsolvable", [clingo.Number(1), clingo.Number(2)]),
            clingo.Function("unsolvable", [clingo.Number(3), clingo.Number(4)]),
            clingo.Function("solvable", [clingo.Number(5), clingo.Number(6)]),  # Should be ignored
        ]

        # Expected output
        expected_unsolvable_states = {(1, 2), (3, 4)}

        # Call the function
        result = unsolvable_states_from_solution(symbols)

        # Print result for verification
        print("Expected:", expected_unsolvable_states)
        print("Result:", result)
        assert result == expected_unsolvable_states, "Test failed!"
        print("Test passed!")

    # Run the test
    verify_unsolvable_states_from_solution()
