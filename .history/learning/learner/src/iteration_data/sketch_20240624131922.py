...
class Sketch:
    ...
    def _verify_unsolvable_states_removed(self, instance_data: InstanceData) -> bool:
        ...
        """
        # Step 1: Identify all unsolvable states
        unsolvable_states = set()
        for s_idx, state in instance_data.state_space.get_states().items():
            if instance_data.is_deadend(s_idx) or not instance_data.is_goal(s_idx):
                unsolvable_states.add(s_idx)
        
        # Step 2: Check if these states are present during the search
        ...
        while queue:
            ...
            if current_idx in unsolvable_states:
                print(f"Unsolvable state found during search: {current_idx}")
                return False
