...
from learning.learner.src.asp.asp_factory import ASPFactory
import clingo  
...
class Sketch:
    ...
    def query_subgoal_distances(asp_factory: ASPFactory, instance_id: int, state_id: int) -> List[int]:
        """
    ...
    def query_d_distances(asp_factory: ASPFactory, instance_id: int, state_id: int) -> List[int]:
        """
    ...
    def verify_unsolvable_states(instance_data: InstanceData, asp_factory: ASPFactory) -> bool:
        """
        ...
        while queue:
            ...
            for successor_idx in successors:
                if successor_idx not in visited:
                    ...
                    # Query subgoal distances for the successor state
                    subgoal_distances = query_subgoal_distances(asp_factory, instance_data.id, successor_idx)
                    
                    if not subgoal_distances:
                        print(colored(f"Warning: No subgoal_distance found for state ({instance_data.id}, {successor_idx}).", "red"))
                        return False
                    
                    # Query d distances for the successor state
                    d_distances = query_d_distances(asp_factory, instance_data.id, successor_idx)
                    
                    if not d_distances:
                        print(colored(f"Warning: No d_distance found for state ({instance_data.id}, {successor_idx}).", "red"))
                        return False
                    
                    # Check the condition: subgoal_dist <= d_dist
                    for subgoal_dist in subgoal_distances:
                        for d_dist in d_distances:
                            ...
                            print(f"Warning: Unsatisfactory d_distance for state ({instance_data.id}, {successor_idx}).")
                    else:
                        print(f"Warning: No subgoal_distance found for state ({instance_data.id}, {successor_idx}).")
