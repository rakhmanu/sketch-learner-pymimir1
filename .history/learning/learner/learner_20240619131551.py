...
def learn_sketch_for_problem_class(
    ...
    if encoding_type == EncodingType.EXPLICIT:
        ...
        for instance_data in instance_datas:
            ...
            for idx, symbols in enumerate(valid_solutions):
                ...
                sketches.add(sketch)  
                # Extract and log unsolvable states
                unsolvable_states = [symbol.arguments[0].number for symbol in symbols if symbol.name == 'unsolvable']
                unsolvable_states_all.update(unsolvable_states)
                if unsolvable_states:
                    logging.warning(f"Unsolvable states in solution {idx + 1}: {unsolvable_states}")
                else:
                    logging.info(f"No unsolvable states in solution {idx + 1}")
    ...
    with change_dir("output"):
        ...
        if unsolvable_states_all:
            print(f"Unsolvable states across all solutions: {unsolvable_states_all}")
        else:
