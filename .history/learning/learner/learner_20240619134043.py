...
def unsolvable_states_from_solution(symbols):
    # Example function to extract unsolvable states from ASP solution symbols
    # Replace with actual logic based on your ASP output format
    unsolvable_states = set()

    for symbol in symbols:
        if isinstance(symbol, clingo.Symbol) and symbol.name == "unsolvable":
            state_id = int(symbol.arguments[0])
            instance_id = int(symbol.arguments[1])
