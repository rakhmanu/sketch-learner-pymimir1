...
def check_sketch_coverage(instance_datas, sketches):
    all_states = set()
    for instance_data in instance_datas:
        all_states.update(instance_data.state_space.get_states().keys())

    covered_states = set()
    for sketch in sketches:
        covered_states.update(sketch.dlplan_policy.states_covered())

    uncovered_states = all_states - covered_states
    if uncovered_states:
        print("Uncovered states:")
        for state_id in uncovered_states:
            print(f"Instance {state_id[1]}, State {state_id[0]}")
        return False
    else:
        print("All states covered by sketches.")
        return True
...
def learn_sketch_for_problem_class(
    ...
    with change_dir("output"):
        ...
        # Check coverage
        coverage = check_sketch_coverage(instance_datas, sketches)
        if not coverage:
            print("Warning: Not all states have a corresponding sketch.")
