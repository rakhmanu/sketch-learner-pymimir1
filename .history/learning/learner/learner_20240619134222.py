    ...
    if encoding_type == EncodingType.EXPLICIT:
        ...
        for instance_data in instance_datas:
            ...
            for idx, symbols in enumerate(valid_solutions):
                ...
                # Collect unsolvable states from this solution
                # Example: Replace with actual logic based on your ASP output format
                for state_id in unsolvable_states_from_solution(symbols):
                    unsolvable_states.add(state_id)

        # Print unsolvable states after all solutions are processed
...
def unsolvable_states_from_solution(symbols):
    ...
    for symbol in symbols:
        if isinstance(symbol, clingo.Symbol) and symbol.name == "unsolvable":
            # Assuming 'unsolvable' has two arguments: state_id and instance_id
            if len(symbol.arguments) >= 2:
                state_id = int(symbol.arguments[0].number)  # Extracting the number from the argument
                instance_id = int(symbol.arguments[1].number)  # Extracting the number from the argument
