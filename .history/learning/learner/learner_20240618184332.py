...
def learn_sketch_for_problem_class(
    ...
    if encoding_type == EncodingType.EXPLICIT:
        ...
        for instance_data in instance_datas:
            ...
            preprocessing_timer.stop()
            asp_timer.resume()
            # Iterate over each state_id in instance_data.state_space.get_states()
            for state_id in instance_data.state_space.get_states():
                # Retrieve the state object corresponding to state_id using initial_s_idxs
                state_data = instance_data.initial_s_idxs[state_id]
                
                # Create ASPFactory instance
                asp_factory = ASPFactory(encoding_type, enable_goal_separating_features, max_num_rules)
                
                # Generate facts for the ASP program
                ...
                # Solve the logic program and collect solutions
